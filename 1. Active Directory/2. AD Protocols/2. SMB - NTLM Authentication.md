...
**SMB session setup associated with NTLM authentication:**

![](../../../../5.%20Lab-writeups/zimage/Pasted%20image%2020250823091646.png)

- (1): Client wants to access resources on SMB server, it starts SMB session.
- (2): Server responds to client with SMB version, the information about encryption and security features such as authentication methods (NTLM, Kerberos).
- (3): After pre-connection (Request/Response), client will choose the authentication method mentioned by (2).
	- **NTLM Authentication:** The server will challenge the client with a nonce value, and the client must respond with NTLM Repsonse.
	- **Kerberos:** The client requests a ticket from a Key Distribution Center (KDC), which verifies the client’s identity and issues a ticket to present to the SMB server.
- (4) **Negotiate Message:** The client sends a message to the server indicating its capabilities, including the SMB version and NTLM version it supports.
- (5) **Challenge Message:** The server responds with a challenge message, which includes a random value (Server Challenge called nonce) and expects the client to prove its identity by encrypting this challenge.

![](../../../../5.%20Lab-writeups/zimage/Pasted%20image%2020250823101236.png)

- (6) **Authenticate Message:** The client responds by encrypting the challenge using a hash of its password, typically found in the `NTLMSSP_AUTH` in the Wireshark capture. This message contains the username and other session details.
	- NTLM Response = Encrypt(ServerChallenge, NT hash-derived keys) - NT hash encrypts Server Challenge.
	- Server will decrypt with NT hash from SAM, then compare NTLM Challenge.

![](../../../../5.%20Lab-writeups/zimage/Pasted%20image%2020250823102149.png)

- **Validation:** The server compares the client’s response with its own expected result. If they match, the client is authenticated and gains access to the requested resource. If not, access is denied.

**SMB session setup associated with NTLM authentication on wireshark:**

![](../../../../5.%20Lab-writeups/zimage/Pasted%20image%2020250823092912.png)
